  ...
  constructor(
    ...
    this.menu.setOnClose((picked) => {
      ...
      if (picked && this.player) {
        if (picked === "Rice") {
          const item = new UncookedRiceItem(this.three, 0, 0, 0);
          this.player.pickup(item);
        }
        else if (picked === "Salmon") {
          const item = new SalmonFishItem(this.three, 0,0,0);
          this.player.pickup(item);
        }
        else if(picked === "Octopus"){
            const item = new OctopusItem(this.three, 0,0,0);
            this.player.pickup(item);
        }
        else if(picked == "Seaweed"){
            const item = new SeaweedItem(this.three, 0,0,0);
            this.player.pickup(item);
        }
        else if(picked == "SeaUrchin"){
            const item = new ClosedSeaUrchinItem(this.three,0,0,0);
            this.player.pickup(item);
        }
        else if(picked == "Cucumber"){
          const item = new CucumberItem(this.three,0,0,0);
          this.player.pickup(item);
        }
