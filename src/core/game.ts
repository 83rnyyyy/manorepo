...
export class Game {
	...
	private async init() {
		...
		const collisionObj = await this.three.loadGLB("/public/testingcol.glb");
		collisionObj.visible = false;
		this.three.scene.add(collisionObj);

		this.world.clear();
		collisionObj.updateWorldMatrix(true, true);
		this.world.fromGraphNode(collisionObj);
		...
		for (let i = 0; i < 3; i++) {
			const plate = new PlateItem(this.three, 0, 0, 0);
			plates?.addPlate(plate)
		}

		const serving = this.stationManager.getByType(Serving);
		serving?.createNewOrder();
	...
	private createStations() {
		...
		const boardAnchor = this.makeAnchor(this.mapObj,  new THREE.Vector3(11, 1, -4.59));
		...
		const sink = new Sink(sinkAnchor, plates);
		sink.halfX = 0.6;
		sink.halfZ = 0.6;
		...
		const serving = new Serving(servingAnchor, sink, this.hud);
		serving.halfX = 6.5;
		serving.halfZ = 2;
